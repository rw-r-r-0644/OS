...
uint8_t * initrd_file_area;				// Initrd file area
...
{	
	...
	initrd_descs		= (initrd_file_desc_t *)((uint32_t)buff + sizeof(initrd_header_t));
	initrd_file_area	= (uint8_t *)((uint32_t)buff + sizeof(initrd_header_t) + (sizeof(initrd_file_desc_t) * initrd_header->entry_count)); 
